...
<div class="flex flex-row w-full">
    <div class="w-56 h-full flex flex-col items-center bg-custom_gray">
        ...
        <div class="flex justify-center">
            <button class=" flex flex-col items-center justify-center  h-56 w-56 bg-green hover:bg-custom_orange" onclick="filterProducts({{ $categorie['category_id'] }})">
    ...
    <div class=" w-full align-items-center  justify-center pt-12 pb-12 gap-4" id="productContainer">
        ...
        <div class="flex shadow-md justify-between flex-col h-128 w-64 rounded-xl bg-custom_green product" data-category-id="{{ $product['category_id'] }}">
            <div class="relative  ">
                <img class="rounded-tl-xl rounded-tr-xl" src="{{ $product['image_url'] }}" alt="food">
                <button onclick="openModal('{{ $product['name'] }}', '{{ $product['description'] }}', '{{ $product['price'] }}', '{{ $product['kcal'] }}', '{{ $product['image_url'] }}')" id="module_open" class="absolute right-0 bottom-0 shadow-xl m-5 bg-white rounded-full z-10">
...
<div id="myModal" onclick="closeModal()" class="fixed inset-0 z-30 flex items-center justify-center bg-black bg-opacity-50 hidden">
    <div class="bg-white gap-4 flex flex-row p-5 rounded-lg shadow-lg w-4/5">
        ...
        <p id="modalDescription" class="mb-4 text-3xl">A blend of acai, banana, and mixed berries topped with granola, chia seeds, and coconut flakes.<p>

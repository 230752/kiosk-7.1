...
<body class="text-gray-900 min-h-screen flex flex-col">
    <main class="flex-grow">
        <a href="{{ route('start') }}" class="flex flex-col items-center justify-center">
            ...
            <div class="flex w-full gap-12 justify-center flex-col mt-28 items-center flex-grow">
                <div class="flex rounded-xl items-center justify-center bg-white h-126 w-120">

                    @php
                    $randomNumber = rand(1000, 9999);
                    $randomLetter = chr(rand(65, 90));
                    $orderNumber = $randomLetter . $randomNumber;
                    @endphp
                    <p class="text-9xl font-bold text-custom_green">{{ $orderNumber }}</p>
                </div>
                <p class="text-8xl font-bold text-custom_gray">This is your number</p>
            </div>
        </a>
    </main>
